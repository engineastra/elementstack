import ProjectBoxSVG from './icons/ProjectBox.svg';
import MachineCodeSVG from './icons/MachineCode.svg';
import ProblemIdeaSVG from './icons/ProblemIdea.svg';
import PeerSVG from './icons/Peer.svg';
import tsxSVG from './icons/tsx.svg';
import tsSVG from './icons/ts.svg';
import jsxSVG from './icons/jsx.svg';
import jsSVG from './icons/js.svg';
import htmlSVG from './icons/html.svg';
import cssSVG from './icons/css.svg';
import jsonSVG from './icons/json.svg';
import { DsaTabType } from './Types';

export type OptionCardConfig = {
  id: number;
  icon: string;
  title: string;
  bgGrad: string;
  textColor: string;
  pathName: string;
};

export const OPTION_CARDS: Record<string, OptionCardConfig> = {
  project: {
    id: 1,
    icon: ProjectBoxSVG,
    title: 'Projects',
    bgGrad: 'project-grad',
    textColor: 'project-500',
    pathName: 'projects',
  },
  machine: {
    id: 2,
    icon: MachineCodeSVG,
    title: 'Machine Coding',
    bgGrad: 'machine-grad',
    textColor: 'machine-500',
    pathName: 'machine-code',
  },
  problems: {
    id: 3,
    icon: ProblemIdeaSVG,
    title: 'DSA Problems',
    bgGrad: 'problems-grad',
    textColor: 'problems-500',
    pathName: 'problems',
  },
  peer: {
    id: 4,
    icon: PeerSVG,
    title: 'Peer Code',
    bgGrad: 'peer-grad',
    textColor: 'peer-500',
    pathName: 'peercode',
  },
};

export const COMMON_COLORS = {
  primary: '#71f163',
  backgroundAccent: '#050816',
  backgroundAccentTransparent: '#050816af',
  card: '#0B1020',
  pannel: '#111827',
  primaryText: '#F9FAFB',
  secondaryText: '#9CA3AF',
  disabled: '#4B5563',
  divider: '#1F2933',
  success: '#22C55E',
  warning: '#F59E0B',
  error: '#EF4444',
  project: { 500: '#ebd197' },
  machine: { 500: '#B69FE9' },
  problems: { 500: '#7EBBF8' },
  peer: { 500: '#22C55E' },
  greenishgrey: '#1F2933',
  codeMirrorBg: '#002351',
  ts: '#2c78c7',
  js: '#F6DE22',
  tsx: '#2cbef5',
  jsx: '#f99e04',
};

export type CreateProjectOptionsConfig = {
  id: number;
  icon: string;
  title: string;
};

export const CREATE_PROJECT_OPTIONS: Record<
  string,
  CreateProjectOptionsConfig
> = {
  tsx: {
    id: 1,
    icon: tsxSVG,
    title: 'React(TS)',
  },
  jsx: {
    id: 2,
    icon: jsxSVG,
    title: 'React(JS)',
  },
  ts: {
    id: 3,
    icon: tsSVG,
    title: 'Typescript',
  },
  js: {
    id: 4,
    icon: jsSVG,
    title: 'Javascript',
  },
};

export const FILE_TYPE_TO_ICON: Record<string, string> = {
  tsx: tsxSVG,
  jsx: jsxSVG,
  ts: tsSVG,
  js: jsSVG,
  html: htmlSVG,
  css: cssSVG,
  json: jsonSVG,
};

export const FILE_TYPE_TO_LANGUAGE: Record<string, string> = {
  tsx: 'typescript',
  jsx: 'javascript',
  ts: 'typescript',
  js: 'javascript',
  html: 'html',
  css: 'css',
  json: 'json',
};

export const EmptyFile = {
  id: '',
  name: '',
  value: '',
  parentFolderId: '',
  type: '',
  language: '',
};

export const PROJECT_THEME_BY_TYPE: Record<
  string,
  { bg: string; text: string; shadow: string }
> = {
  tsx: {
    bg: '',
    text: '#2cbef5',
    shadow: 'inset 0 0 10px #2cbef5',
  },
  jsx: {
    bg: '',
    text: '#f99e04',
    shadow: 'inset 0 0 10px #f99e04',
  },
  ts: {
    bg: '',
    text: '#2c78c7',
    shadow: 'inset 0 0 10px #2c78c7',
  },
  js: {
    bg: '',
    text: '#F6DE22',
    shadow: 'inset 0 0 10px rgba(246, 223, 29, 0.6)',
  },
  other: {
    bg: '',
    text: '#ff9700',
    shadow: 'inset 0 0 10px #ff9700',
  },
};

export const MACHINE_TOPICS_TO_FEATURES: Array<{
  id: string;
  topic: string;
  features: string[];
}> = [
  {
    id: 't1-state-management',
    topic: 'State Management',
    features: [
      'State Management',
      'Complex State',
      'Global State',
      'Context API',
      'Field State Management',
      'Flexible State',
      'Quiz Management',
      'Cart Management',
      'Vote Tracking',
      'Game Logic',
    ],
  },
  {
    id: 't2-data-persistence',
    topic: 'Data Persistence & Storage',
    features: [
      'Persistence',
      'Data Caching',
      'Offline Support',
      'Auto-save',
      'History',
      'History Management',
      'History Storage',
      'Persistence',
      'Favorites',
      'Watchlist',
    ],
  },
  {
    id: 't3-crud-data-ops',
    topic: 'CRUD & Data Operations',
    features: [
      'CRUD Operations',
      'Edit/Delete',
      'Category Management',
      'Cart Management',
      'Vote Tracking',
      'Editing/Deletion',
      'Bulk Actions',
      'Add/Delete',
      'Create Polls',
      'Folder Management',
    ],
  },
  {
    id: 't4-ui-interactions',
    topic: 'User Interaction & Input Handling',
    features: [
      'Interactivity',
      'Hover Effects',
      'Click-outside Detection',
      'Touch Support',
      'Touch Gestures',
      'Swipe Gestures',
      'Smooth Dragging',
      'Zoom Functionality',
      'Drag-Drop Support',
    ],
  },
  {
    id: 't5-keyboard-access',
    topic: 'Keyboard & Accessibility',
    features: [
      'Keyboard Navigation',
      'Keyboard Support',
      'Keyboard Shortcuts',
      'Accessibility',
      'Focus Trap',
      'ARIA Support',
      'Semantic HTML',
      'Callback Props',
      'Dynamic Generation',
      'Responsive Layout',
    ],
  },
  {
    id: 't6-animations-motion',
    topic: 'Animations & Motion',
    features: [
      'Animations',
      'Smooth Animations',
      'Slide Animations',
      'Smooth Animation',
      'CSS Animation',
      'Backdrop',
      'Transitions',
      'Character Animation',
      'Multiple Transitions',
      'Custom Easing',
      'Zoom Effects',
    ],
  },
  {
    id: 't7-performance',
    topic: 'Performance Optimization',
    features: [
      'Performance',
      'Virtual Scrolling',
      'Lazy Loading',
      'IntersectionObserver',
      'Image Optimization',
      'Performance Optimization',
      'Large Dataset Support',
      'Dynamic Heights',
      'Scroll-to-Index',
    ],
  },
  {
    id: 't8-search-filtering',
    topic: 'Search, Filtering & Sorting',
    features: [
      'Filtering',
      'Search',
      'Search Filtering',
      'Advanced Search',
      'Sorting/Filtering',
      'Debounced Search',
      'Date Filtering',
      'Tag Display',
      'Grouping',
      'Multiple Filters',
      'Smart Ellipsis',
      'Boundary Handling',
    ],
  },
  {
    id: 't9-api-network',
    topic: 'API & Network Integration',
    features: [
      'API Integration',
      'WebSocket Integration',
      'Real-time Updates',
      'Geolocation',
      'Language Detection',
      'Caching',
      'Data Caching',
      'Error Handling',
      'Typing Indicators',
      'Message Status',
      'Infinite Scroll',
      'Request Interception',
      'Filtering',
      'Performance Metrics',
    ],
  },
  {
    id: 't10-error-loading',
    topic: 'Error & Loading Handling',
    features: [
      'Error Handling',
      'Loading States',
      'Error Display',
      'Progress Tracking',
      'File Validation',
      'Chunked Upload',
      'Debouncing',
      'Load Functionality',
      'Loading Skeletons',
    ],
  },
  {
    id: 't11-drag-drop',
    topic: 'Drag & Drop Systems',
    features: [
      'Drag & Drop',
      'Nested Items',
      'Smart Positioning',
      'Flexible Positioning',
      'Node Creation',
      'Connection Management',
      'Zoom/Pan',
      'Draggable Dividers',
      'Nested Splits',
      'Collapse/Expand',
      'Draggable Tasks',
    ],
  },
  {
    id: 't12-forms-validation',
    topic: 'Forms & Validation',
    features: [
      'Custom Hooks',
      'Async Validation',
      'Reusability',
      'Validation Schemas',
      'Inline Editing',
      'Validation',
      'Requirements Check',
      'Pattern Detection',
      'Suggestions',
      'Conditional Fields',
      'Multi-step',
      'Field Dependencies',
      'Progress Indicator',
      'State Persistence',
    ],
  },
  {
    id: 't13-navigation-layout',
    topic: 'Navigation & Layout',
    features: [
      'Responsive Design',
      'Responsive',
      'Smart Ellipsis',
      'Boundary Handling',
      'Dynamic Generation',
      'Multiple Directions',
      'Responsive Menu',
      'Hamburger Menu',
      'Sticky Header',
      'Smooth Scrolling',
      'Section Highlighting',
      'Styling Flexibility',
      'Responsive Images',
      'Responsive Layout',
    ],
  },
  {
    id: 't14-media',
    topic: 'Media & Rich Content',
    features: [
      'Video API',
      'Audio API',
      'Subtitles',
      'Visualization',
      'Image Gallery',
      'Full-screen Display',
      'Navigation',
      'Keyboard Shortcuts',
      'Thumbnail Navigation',
      'Zoom Functionality',
      'Multiple Tools',
      'Layer System',
      'Audio Visualization',
      'Metadata Display',
    ],
  },
  {
    id: 't15-editor-tools',
    topic: 'Editors & Developer Tools',
    features: [
      'Syntax Highlighting',
      'Line Numbers',
      'Live Preview',
      'Toolbar',
      'Custom Styling',
      'Expand/Collapse',
      'Search Functionality',
      'Theme Support',
      'Side-by-Side View',
      'Change Highlighting',
      'Merge Capabilities',
      'Export Functionality',
    ],
  },
  {
    id: 't16-charts-analytics',
    topic: 'Charts & Analytics',
    features: [
      'Data Visualization',
      'Results Visualization',
      'Progress Charts',
      'Statistics',
      'Results Analysis',
      'Metrics Display',
      'WIP Limits',
      'Activity Tracking',
      'Analytics',
      'Progress Tracking',
      'Burn-down Chart',
    ],
  },
  {
    id: 't17-notifications',
    topic: 'Notifications & Feedback',
    features: [
      'Notifications',
      'Auto-dismiss',
      'Stacking',
      'Message Status',
      'Priority Queue',
      'Grouping',
      'Undo Support',
      'Web Push API',
      'Sound Effects',
      'Completion Percentage',
    ],
  },
  {
    id: 't18-security-utils',
    topic: 'Security & Utilities',
    features: [
      'Password Generation',
      'Strength Indicator',
      'Copy Functionality',
      'File Validation',
      'Strength Meter',
      'Encryption Simulation',
      'Master Password',
      'Copy-to-Clipboard',
      'Unique Short Codes',
      'QR Code',
    ],
  },
  {
    id: 't19-productivity',
    topic: 'Productivity Features',
    features: [
      'Undo/Redo',
      'Focus Mode',
      'Recurring Tasks',
      'Priority System',
      'Statistics',
      'Move History',
      'Difficulty Levels',
      'Lap Tracking',
      'Proper Formatting',
      'Export Functionality',
      'Collections',
      'Snippet Storage',
      'Search & Filter',
      'Recurring Tasks',
      'Categories & Tags',
      'Focus Mode',
    ],
  },
  {
    id: 't20-deployment-meta',
    topic: 'Export, Deployment & Meta',
    features: [
      'Export Functionality',
      'Export/Deploy',
      'Theme Support',
      'SEO Optimization',
      'Customization',
      'Multiple Templates',
      'Template System',
      'Preview',
      'Styling',
      'Props-based Customization',
      'Easy Integration',
      'Multiple Variants',
      'Responsive',
      'PDF Export',
    ],
  },
  {
    id: 't21-data-management',
    topic: 'Data Management & Structures',
    features: [
      'Cell Editing',
      'Formulas',
      'Copy/Paste',
      'Category Management',
      'Nested Comments',
      'Voting',
      'Sorting',
      'User Mentions',
      'Data Structure Management',
      'Nested Menu Items',
    ],
  },
  {
    id: 't22-advanced-interactions',
    topic: 'Advanced User Interactions',
    features: [
      'Progress Rings',
      'Range Selection',
      'Keyboard Navigation',
      'Value Labels',
      'Edge Cases',
      'Dual Handles',
      'Motion',
      'Drag Operations',
      'Dependencies',
      'Move Tracking',
      'Card Matching',
      'Share Functionality',
      'Email Preview',
      'Responsive Layout',
    ],
  },
  {
    id: 't23-game-entertainment',
    topic: 'Game & Entertainment Features',
    features: [
      'Game Logic',
      'Scoring',
      'Leaderboard',
      'Animations',
      'Difficulty Levels',
      'AI Opponent',
      'Statistics',
      'Difficulty Levels',
      'Timer',
      'Card Matching',
      'Move Tracking',
      'Sound Effects',
    ],
  },
  {
    id: 't24-real-time-collaboration',
    topic: 'Real-Time & Collaboration',
    features: [
      'WebSocket Integration',
      'Real-time Updates',
      'Typing Indicators',
      'Message Status',
      'Offline Support',
      'WIP Limits',
      'Activity Tracking',
      'Real-time Updates',
      'Priority Queue',
      'Concurrent Requests',
      'URL Sync',
      'Filter Presets',
    ],
  },
  {
    id: 't25-conversion-utilities',
    topic: 'Conversion & Format Utilities',
    features: [
      'Format Conversion',
      'Multiple Formats',
      'Gradient Selector',
      'Color Palette',
      'Contrast Checker',
      'Unit Conversion',
      'Shopping List',
      'Calculations',
      'Ingredient Scaling',
      'Language Detection',
      'Text-to-Speech',
    ],
  },
  {
    id: 't26-recommendations-personalization',
    topic: 'Recommendations & Personalization',
    features: [
      'Recommendations',
      'Color Harmony',
      'Color Naming',
      'Similar Recommendations',
      'Saved Filter Presets',
      'Autocomplete',
      'API Integration',
      'Debouncing',
      'Caching',
      'Real-time Feedback',
    ],
  },
  {
    id: 't27-advanced-layouts',
    topic: 'Advanced Layout Systems',
    features: [
      'Swimlane Organization',
      'Nested Splits',
      'Nested Items',
      'Expand/Collapse',
      'Lazy Loading',
      'Virtual Scrolling',
      'Dynamic Heights',
      'Hierarchy',
      'Tree Structure',
      'Recursive Rendering',
    ],
  },
  {
    id: 't28-content-display',
    topic: 'Content Display & Rendering',
    features: [
      'Full-screen Display',
      'Image Optimization',
      'Blur-up Effect',
      'Preview',
      'Rich Text',
      'Syntax Highlighting',
      'Theme Support',
      'Expand/Collapse',
      'Inline Editing',
      'Multi-language Support',
      'Language Detection',
    ],
  },
];

export const DSA_CONSTANTS = {
  companyTags: ['Amazon', 'Apple', 'Facebook', 'Google', 'Meta', 'Microsoft'],
  keyConcepts: [
    '2D Array',
    '2D Dynamic Programming',
    '2D Grid',
    'Arithmetic',
    'Array',
    'Array Iteration',
    'Array Manipulation',
    'Array Partitioning',
    'Array Traversal',
    'BFS',
    'BFS/DFS',
    'BST',
    'Backtracking',
    'Binary Search',
    'Binary Tree',
    'Bit Manipulation',
    'Cache Design',
    'Carry Handling',
    'Case Analysis',
    'Character Counting',
    'Circuit Problem',
    'Coin Change',
    'Combinations',
    'Connected Components',
    'Constraint',
    'Constraint Satisfaction',
    'Cycle Detection',
    'DFS',
    'DFS/BFS',
    'Deep Copy',
    'Deque',
    'Deserialization',
    'Design',
    'Dijkstra',
    'Distance Calculation',
    'Divide and Conquer',
    'Duplicate Handling',
    'Dynamic Programming',
    'Edit Distance',
    'Eulerian Path',
    'Exponentiation',
    'Expression Evaluation',
    'Fibonacci',
    'Frequency Counting',
    'Geometry',
    'Graph',
    'Greedy',
    'Grouping',
    'Hash Map',
    'Hash Set',
    'Heap',
    'Height Calculation',
    'In-order Traversal',
    'In-place',
    'Integer Handling',
    'Interval',
    'Interval DP',
    'Interval Manipulation',
    'Interval Merging',
    'Knapsack',
    'LCA',
    'Level-wise',
    'Linked List',
    'List Manipulation',
    'Mapping',
    'Math',
    'Matrix Manipulation',
    'Matrix Traversal',
    'Memoization',
    'Merging',
    'Minimum Spanning Tree',
    'Monotonic Queue',
    'Monotonic Stack',
    'Multi-source',
    'Multiplication',
    'O(1) Operations',
    'O(n) Algorithm',
    'Optimization',
    'Palindrome',
    'Partition',
    'Path Calculation',
    'Path Counting',
    'Path Sum',
    'Path Tracking',
    'Pattern Matching',
    'Permutation',
    'Permutations',
    'Pointer Manipulation',
    'Pointer Movement',
    'Pointers',
    'Prefix/Suffix',
    'Priority Queue',
    'Product Tracking',
    'Queue',
    'QuickSelect',
    'Recursion',
    'Reverse Thinking',
    'Rotated Array',
    'Sequence Detection',
    'Serialization',
    'Shortest Path',
    'Simulation',
    'Single Pass',
    'Sliding Window',
    'Sorted Array',
    'Sorting',
    'Stack',
    'State Machine',
    'Stream Processing',
    'String',
    'String Building',
    'String Decoding',
    'String Encoding',
    'String Interleaving',
    'String Manipulation',
    'String Sorting',
    'String Validation',
    'Subsequence',
    'Sudoku Rules',
    'Topological Sort',
    'Tree',
    'Tree Comparison',
    'Tree Construction',
    'Tree Traversal',
    'Trie',
    'Two Heaps',
    'Two Pointers',
    'Two Pointers Concept',
    'Union-Find',
    'Wildcard',
    'Word Segmentation',
    'Word Transformation',
    'XOR',
  ],
  relatedTopics: [
    'array',
    'arrays',
    'backtracking',
    'binary-search',
    'binary-search-tree',
    'bit-manipulation',
    'breadth-first-search',
    'depth-first-search',
    'design',
    'divide-and-conquer',
    'dynamic-programming',
    'geometry',
    'graph',
    'greedy',
    'hash-table',
    'heap',
    'interval',
    'linked-list',
    'math',
    'matrix',
    'queue',
    'recursion',
    'sliding-window',
    'sorting',
    'stack',
    'string',
    'topological-sort',
    'tree',
    'trie',
    'two-pointers',
    'union-find',
  ],
  categories: [
    '2D Dynamic Programming',
    'Advanced Graph',
    'Array Manipulation',
    'Backtracking',
    'Binary Search',
    'Binary Tree',
    'Bit Manipulation',
    'Dynamic Programming',
    'Graph',
    'Greedy',
    'Hash Map',
    'Hash Set',
    'Heap',
    'Interval',
    'Linked List',
    'Math',
    'Sliding Window',
    'Stack',
    'String Encoding',
    'Trie',
    'Two Pointers',
  ],
};

export const DsaTabs: Record<string, DsaTabType> = {
  Desc: { name: 'Description', isLeft: true },
  Code: { name: 'Code', isLeft: false },
};
